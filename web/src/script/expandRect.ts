...
export const rectArea = (rect: LatLongRect) => {
    ...
    const latDistortion = Math.cos(
        (((rect.minLat + rect.maxLat) / 2) * Math.PI) / 180,
    )
...
export const removeContainedRects = (rects: ExpandingRect[]): ExpandingRect[] =>
    rects.filter(
        (rect) =>
            !rects.some(
                (r) => r !== rect && isContainedByRect(r.rect, rect.rect),
            ),
    )
...
export const expandRect = (
    ...
    do {
        ...
        isContainedByRect(tmpResult, bounding) &&
        !rectangleIntersectsPolygon(tmpResult, polygon)
    )

...
import { boundingRect, type ExpandingRect } from "./expandRect"
...
load(
    ...
    const features: Feature[] = typedUnvalidatedResult.data.map((feature) => {
        ...
        if (typeof clusterName !== "string")
            ...
            parsedFeature.geometry?.type === "Polygon" ||
            parsedFeature.geometry?.type === "MultiPolygon"
        ) {
            ...
            if (parsedFeature.geometry.type === "Polygon") {
                parsedFeature.geometry.coordinates.forEach((polygon) => {
                    ...
                    polygonRects.push(
                        approximateLargestAlignedRectangle(
            ...
            // It means the geographic region is disconnected. We can still just look for the largest single rectangle.
                ...
                for (const multiPolygon of parsedFeature.geometry.coordinates) {
                    for (const polygon of multiPolygon) {
                        const bounding = boundingRect(
                            polygon as [number, number][],
                        )
                        ...
                        if (
                            bounding.maxLat - bounding.minLat < 0.2 ||
                            bounding.maxLong - bounding.minLong < 0.2
                        ) {
                            console.debug(
                                `Skipping tiny polygon for ${clusterName} with ${polygon.length} points and min dimension ${minDim}`,
                            )
                        } else {
                            try {
                                polygonRects.push(
                                ...
                                console.error(
                                    `Error processing ${clusterName}, min dimension ${minDim} with ${polygon.length} points: ${e}`,
                                )
                            }
                        }
                    }
            ...
            if (polygonRects.length === 0)
                console.error(
                    `No rectangles found for ${parsedFeature.properties?.Cluster}`,
                )
            else
                largestClusterRects[clusterName] = polygonRects.reduce(
    ...
    const validatedShapefile: ShapeFilePlusLargestRects = {
        ...rest,
        features,
        largestClusterRects,
    }
    ...
    await writeFile(
        OUTPUT_FILENAME,
        JSON.stringify(validatedShapefile, null, 1),
    )
